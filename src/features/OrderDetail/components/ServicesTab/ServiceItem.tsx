		...
		if (IS_DISCOUNT) {
			...
			history.push({
				pathname: `/chi-tiet-giam-gia/${slugify(
					IS_DISCOUNT.productable.service_name || IS_DISCOUNT.productable.product_name
				)}`,
					...
					<div className="flex-row-sp item-bottom">
						{serviceItem?.discount_value > 0 ? (
							<div className="flex-row">
								<span className="price" style={{ color: 'var(--orange)' }}>
									{formatPrice(service?.price - serviceItem?.discount_value)}đ
								</span>
								<span className="old-price">{formatPrice(service?.price)}</span>
							</div>
						) : (
							<span className="price">{formatPrice(serviceItem.base_price)}đ</span>
						)}
						<div className="flex-row item-button">
							<ButtonCus onClick={handleDetailService} text={t('order.watch_info')} />
							<ButtonCus onClick={handleAddCart} text="Pre-Order" />
						</div>
					</div>
				</div>
			</div>
